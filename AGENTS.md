...
- **Mandatory Staff Setup & Author Portal Editorial Contact**: First-time login setup (`/change-password`) enforces active email, country code selector, and WhatsApp phone number with explicit English explanation banner. Author Portal displays a dedicated, simplified Editorial Contact card (Name, Email, WhatsApp click-to-chat `wa.me`) position-optimized after Submission Details on mobile viewports.
- **Configurable Editorial Guidelines URL**: Conference Admin can configure an external or Google Drive guidelines link in Conference Settings (`editorial_guidelines_url`), which renders a prominent English reference banner inside the Author Portal checklist monitoring accordion.
...
- `2026_08_28_000100_add_editorial_base_file_to_author_revisions.php`
...
- **[COMPLETED] Branch `feat/local-supabase-docker` Merged to `agent/build-paperflow`**:
  - Implementasi Supabase Lokal / Docker PostgreSQL (`127.0.0.1:54322` via Docker), perintah `php artisan paperflow:switch-supabase {mode=local|cloud}`, 3 file launcher `.bat` (`start-paperflow.bat`, `start-paperflow-ngrok.bat`, `start-paperflow-ngrok-local.bat`), serta pembersihan otomatis proses `ngrok.exe` gantung telah di-merge secara penuh dan teruji 100% pada branch utama `agent/build-paperflow`.
