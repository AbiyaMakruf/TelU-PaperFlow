<x-layouts.guest title="Ganti password · Paperflow">
    ...
    <form method="POST" action="{{ route('password.change.update') }}" class="space-y-5">
        ...
        <div><label class="form-label" for="email">Email aktif</label><input class="form-input" id="email" type="email" name="email" value="{{ old('email', auth()->user()->email) }}" required autocomplete="email"><p class="mt-1 text-xs text-muted">Email ini dapat digunakan untuk login dan pemulihan password.</p></div>
