<x-layouts.app :title="$submission->paper_code" heading="Paper Details">
    <a class="back-link" href="{{ route('submissions.index') }}">&larr; Back to papers</a>
        ...
        <div class="min-w-0">
            ...
            <div class="mt-1 flex items-start gap-1.5 min-w-0"><p class="page-subtitle leading-snug break-words min-w-0 flex-1">{{ $submission->title }}</p><button type="button" data-copy="{{ $submission->title }}" onclick="window.copyPaperflowText(this.dataset.copy, 'Paper title copied.')" class="mt-0.5 inline-flex size-5 shrink-0 items-center justify-center overflow-visible rounded text-slate-400 hover:text-orange transition" title="Copy Paper Title" aria-label="Copy Paper Title"><svg class="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2 2v1"/></svg></button></div>
        </div>
