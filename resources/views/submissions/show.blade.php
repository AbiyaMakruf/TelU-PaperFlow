<x-layouts.app :title="$submission->paper_code" heading="Paper Details">
    <a class="back-link" href="{{ route('submissions.index') }}">&larr; Back to papers</a>
        ...
        <div class="min-w-0">
            ...
            <h1 class="page-title leading-tight break-words">{{ $submission->paper_id ?: $submission->paper_code }} <button type="button" data-copy="{{ $submission->paper_id ?: $submission->paper_code }}" onclick="window.copyPaperflowText(this.dataset.copy, 'Paper ID copied.')" class="inline-flex size-5 shrink-0 translate-y-[-0.08em] items-center justify-center rounded text-slate-400 hover:text-orange transition" title="Copy Paper ID" aria-label="Copy Paper ID"><svg class="block size-4 overflow-visible" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg></button></h1>
            <div class="mt-1 flex items-start gap-1.5 min-w-0"><p class="page-subtitle leading-snug break-words min-w-0 flex-1">{{ $submission->title }}</p><button type="button" data-copy="{{ $submission->title }}" onclick="window.copyPaperflowText(this.dataset.copy, 'Paper title copied.')" class="mt-0.5 inline-flex size-5 shrink-0 items-center justify-center rounded text-slate-400 hover:text-orange transition" title="Copy Paper Title" aria-label="Copy Paper Title"><svg class="block size-4 overflow-visible" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg></button></div>
        </div>
                            ...
                            <ol class="grid gap-2 text-xs text-slate-700 sm:grid-cols-2">
                                @foreach($edasAuthorDetails ?? $edasAuthors as $author)
                                    ...
                                    <li class="rounded-lg border border-sky-100 bg-white px-3 py-2 space-y-0.5">
                                        ...
                                        @if($authorEmail)
                                            <div class="text-[11px] text-sky-700 truncate" title="{{ $authorEmail }}">
                                                <a href="mailto:{{ $authorEmail }}" class="hover:underline">{{ $authorEmail }}</a>
                                            </div>
                                        @endif
                                    </li>
                                @endforeach
