        ...
        <aside class="space-y-6">
            ...
            <section class="card p-6"><h2 class="font-black text-navy">Aksi tahap</h2><div class="mt-4 space-y-4">
                ...
                @can('reviewerReview', $submission)
                    ...
                    @endif
                    @if($submission->status === \App\Enums\SubmissionStatus::ReadyForEdas)
                        <form method="POST" action="{{ route('submissions.advance', $submission) }}" class="space-y-3 border-t border-navy/10 pt-4">@csrf<input type="hidden" name="action" value="done"><input class="form-input" name="edas_reference" placeholder="EDAS ID / reference"><textarea class="form-input min-h-20 py-3" name="note" placeholder="Catatan EDAS"></textarea><button class="btn btn-primary w-full">Tandai selesai</button></form>
                    @endif
                @endcan
            </div></section>
