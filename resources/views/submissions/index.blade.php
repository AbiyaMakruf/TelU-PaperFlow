<x-layouts.app title="Paper" heading="Paper">
    ...
    <div x-data="{ mobileFilterOpen: {{ request()->query() ? 'true' : 'false' }} }" class="card mt-7 p-4 sm:p-5">
        <button type="button" @click="mobileFilterOpen = !mobileFilterOpen" class="flex w-full items-center justify-between font-bold text-navy md:hidden">
            <span class="flex items-center gap-2 text-sm">
                🔍 Filter & Pencarian
                @if(request()->query())
                    <span class="badge badge-primary text-[10px]">Aktif</span>
                @endif
            </span>
            <span class="text-xs text-orange" x-text="mobileFilterOpen ? 'Tutup −' : 'Buka +'"></span>
        </button>
        ...
        <form x-show="mobileFilterOpen || window.innerWidth >= 768" x-collapse class="mt-4 md:mt-0 space-y-4">
            <div class="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
                <div>
                    ...
                    <input class="form-input" name="search" value="{{ request('search') }}" placeholder="Cari kode, judul, author...">
                </div>
                <div>
                ...
                <div>
                    <label class="form-label">Masuk Dari</label>
                ...
                <div>
                    ...
                    <label class="check-row h-12 py-0 px-4 flex items-center cursor-pointer">
                        ...
                        <span class="text-xs font-extrabold text-navy">Hanya Overdue</span>
                    </label>
                </div>
            </div>
            <div class="flex flex-wrap items-center gap-3 pt-3 border-t border-navy/8">
                <button class="btn btn-primary px-6 w-full sm:w-auto">Terapkan filter</button>
                @if(request()->query())
    ...
    <div x-data="{ selected: [], bulkAction: '', activePic: null, staffMap: {{ json_encode($staffData) }} }">
        ...
        <div class="card mt-6 overflow-hidden">
            ...
            <div class="hidden overflow-x-auto md:block">
                <table class="data-table">
                    ...
                    @forelse ($submissions as $submission)
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @else
                                    <p class="text-muted text-xs">Belum ada editor</p>
                                @endif
                                ...
                                @else
                                    <p class="mt-1 text-xs text-muted">Reviewer: -</p>
                                @endif
                            </td>
                    ...
                    @empty
                        <tbody><tr><td colspan="8" class="py-12 text-center text-muted">Belum ada paper yang sesuai filter.</td></tr></tbody>
                    @endforelse
                </table>
            </div>
                ...
                @forelse($submissions as $submission)
                    <article x-data="{ open: false }" class="p-4 space-y-3">
                        ...
                        <div>
                            ...
                            @if($submission->is_flagged_duplicate)
                                <span class="mt-1 inline-block rounded bg-rose-100 px-2 py-0.5 text-[10px] font-bold text-rose-700">⚠️ Potensi Duplikat</span>
                            @endif
                        </div>

                        <div class="flex flex-wrap items-center justify-between gap-2 pt-1">
                            <span class="badge badge-{{ $submission->status->color() }} text-xs">{{ $submission->status->label() }}</span>
                            <span class="text-xs text-muted font-medium">
                                Masuk: {{ $submission->submitted_at?->format('d M Y') ?? '-' }}
                            </span>
                        </div>
                        ...
                        <div class="rounded-xl bg-slate-50 border border-slate-200/80 p-3 text-xs grid grid-cols-1 sm:grid-cols-2 gap-2">
                            <div class="min-w-0">
                                ...
                                @else
                                    <p class="mt-0.5 text-muted">Belum ada editor</p>
                                @endif
                            </div>
                            <div class="min-w-0">
                                ...
                                @else
                                    <p class="mt-0.5 text-muted">Belum ada reviewer</p>
                                @endif
                            </div>
                        </div>

                        <div class="flex items-center justify-between gap-3 pt-1">
                            <button type="button" class="text-xs font-bold text-orange" x-on:click="open = !open" x-text="open ? 'Tutup ringkasan −' : 'Ringkasan info +'">
                                Ringkasan info +
                            </button>
                        ...
                        <div x-cloak x-show="open" x-collapse class="rounded-xl bg-warm p-4 text-xs space-y-2.5">
                            <div><span class="text-muted font-bold block">Corresponding author</span><p class="font-semibold text-navy">{{ $submission->corresponding_author_name }} ({{ $submission->corresponding_author_email }})</p></div>
                            <div><span class="text-muted font-bold block">Format naskah</span><p class="font-semibold text-navy">{{ $submission->manuscript_format === 'latex' ? 'LaTeX (ZIP)' : ($submission->manuscript_format === 'docx' ? 'Microsoft Word (DOCX)' : 'Belum dikonfirmasi') }}</p></div>
        ...
        <!-- PIC Detail Modal (Mobile Responsive) -->
            ...
            <div class="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl bg-white p-5 sm:p-6 shadow-2xl my-auto" @click.away="activePic = null">
                <div class="flex items-start justify-between border-b border-navy/10 pb-4">
                    <div class="min-w-0 pr-2">
                        <p class="eyebrow">Informasi Staf & Workload</p>
                ...
                <div class="mt-4 space-y-4 text-sm">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 rounded-xl bg-warm p-4">
                        ...
                        <div class="min-w-0">
                            <span class="text-xs font-bold text-muted block">WhatsApp / No. HP</span>
                            ...
                            <template x-if="!activePic?.whatsapp">
                                <p class="text-muted text-xs">Belum diatur</p>
                            </template>
                        </div>
                    </div>

                    <div>
                        <h4 class="font-bold text-navy text-xs uppercase tracking-wider mb-2">Keanggotaan Conference & Penugasan</h4>
                        <div class="space-y-2 max-h-48 overflow-y-auto pr-1">
                            <template x-for="mem in activePic?.memberships" :key="mem.conference_slug">
                                <div class="flex items-center justify-between rounded-xl border border-navy/10 p-3 text-xs">
                                    ...
                                    <div class="text-right shrink-0">
                                        <p class="font-black text-navy text-sm" x-text="mem.total_papers_count + ' Paper'"></p>
                            ...
                            <template x-if="!activePic?.memberships?.length">
                                <p class="text-xs text-muted">Belum terdaftar di conference aktif.</p>
                            </template>
                        </div>
                    </div>

                    <div class="rounded-xl border border-navy/10 p-4 bg-slate-50 flex items-center justify-between">
                        <span class="font-bold text-navy text-xs uppercase">Total Paper Ditugaskan (Keseluruhan)</span>
                        <span class="badge badge-primary text-sm font-black px-3 py-1" x-text="(activePic?.total_assigned_papers || 0) + ' Paper'"></span>
                    </div>
                </div>
                <div class="mt-6 flex justify-end">
                    <button type="button" @click="activePic = null" class="btn btn-secondary text-xs px-5 py-2">Tutup</button>
                </div>
            </div>
        </div>
