<div class="card p-4 sm:p-6 min-w-0">
    ...
    @if($submission->editor)
        ...
        <div class="mt-4 space-y-3.5 text-xs">
            <div class="space-y-1">
                <span class="text-[11px] font-bold uppercase tracking-wider text-muted block">Assigned Editor</span>
            ...
            <div class="pt-3 border-t border-slate-100 space-y-2">
                ...
                @if($editorWaDigits)
                    <div class="space-y-2">
                        <span class="font-bold text-navy text-xs block">{{ $editorWa }}</span>
                        <a href="https://wa.me/{{ $editorWaDigits }}?text={{ rawurlencode('Hello ' . $submission->editor->name . ', I am the corresponding author of Paper ID ' . $submission->paper_id . ' (' . $submission->title . ').') }}" 
                           ...
                           class="w-full inline-flex items-center justify-center gap-2 py-2 px-3 text-xs font-extrabold bg-[#25D366] text-white hover:bg-[#1faa52] rounded-xl shadow-2xs transition text-center">
                            ...
                            <span>Contact Editor via WhatsApp</span>
                        </a>
                    </div>
                @else
