    ...
    <div class="min-h-screen lg:grid lg:grid-cols-[260px_1fr]">
        <aside class="hidden bg-navy px-5 py-7 text-white lg:flex lg:flex-col">
            ...
            <div class="mt-auto rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <form method="POST" action="{{ route('logout') }}" class="mt-4">@csrf<button class="text-xs font-bold text-orange hover:text-white">Keluar →</button></form>
            </div>
        </aside>
        <div class="min-w-0">
            <header class="flex h-18 items-center justify-between border-b border-navy/10 bg-white/80 px-5 backdrop-blur sm:px-8 lg:px-10">
                ...
                <div class="flex items-center gap-3">@if(auth()->user()->isSuperAdmin())<span class="badge badge-warning">Superadmin</span>@endif<span class="grid size-10 place-items-center rounded-full bg-navy font-bold text-white">{{ strtoupper(substr(auth()->user()->name, 0, 1)) }}</span></div>
            </header>
