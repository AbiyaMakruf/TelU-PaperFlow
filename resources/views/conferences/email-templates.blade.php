    ...
    <div class="w-full space-y-6">
        ...
        <form method="POST" action="{{ route('conferences.email-templates.update', $conference) }}" class="mt-7 space-y-5">
            ...
            <section class="card p-6">
                <h2 class="font-black text-navy">Sender Identity</h2>
                <p class="mt-1 text-sm text-muted">This name will appear in author inboxes. The global sender address remains {{ config('mail.from.address') }}.</p>
                <label class="mt-5 block">
                    <span class="form-label">Sender Display Name</span>
                    <input class="form-input" name="email_sender_name" value="{{ old('email_sender_name', $conference->email_sender_name ?: $conference->name) }}" maxlength="255" placeholder="e.g. {{ $conference->name }} Editorial Team">
                </label>
                <label class="mt-5 block">
                    <span class="form-label">Default Conference CC</span>
                    <input class="form-input" name="conference_default_cc" value="{{ old('conference_default_cc', implode(', ', $conference->defaultCc())) }}" placeholder="chair@example.com, publication@example.com">
                    <span class="mt-1 block text-xs text-muted">Populates automatically in editorial forms; can still be removed before dispatching.</span>
                </label>
            </section>
                ...
                <section class="card p-6" x-data="{
                    ...
                    render(text) {
                        return text.replace(/\{\{conference\}\}/g, '{{ $conference->name }}')
                                   ...
                                   .replace(/\{\{feedback\}\}/g, 'Please update reference formatting to adhere to IEEE guidelines.')
                    ...
                    <div class="flex items-center justify-between gap-4 flex-wrap">
                        ...
                        <div class="flex items-center gap-3">
                            <button type="button" @click="showPreview = !showPreview" class="btn btn-secondary text-xs py-1.5 px-3">
                                <span x-text="showPreview ? 'Hide Preview' : '👁️ Live Preview'"></span>
                            </button>
                    ...
                    <div class="mt-4 flex flex-col gap-3 rounded-xl border border-orange/20 bg-orange/5 p-4 sm:flex-row sm:items-end">
                        <label class="flex-1">
                            <span class="form-label">Send Test Preview To</span>
                            <input class="form-input" type="email" name="test_recipients[{{ $template->id }}]" value="{{ old('test_recipients.'.$template->id, auth()->user()->email) }}">
                        </label>
                        <button class="btn btn-secondary" name="test_template_id" value="{{ $template->id }}" formaction="{{ route('conferences.email-templates.test', $conference) }}">Send Test Email</button>
                    </div>
                </section>
            @endforeach
            <div class="sticky bottom-4 flex justify-end">
                <button class="btn btn-primary shadow-xl">Save All Templates</button>
            </div>
        </form>
    </div>
</x-layouts.app>
