<x-layouts.app title="Conferences · Paperflow" heading="Conferences">
    <div x-data="{ deleteTarget: null, deleting: false }">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                @can('delete', $conference)
                                    <button type="button" 
                                            ...
                                            data-url="{{ route('conferences.destroy', $conference) }}" 
                                            @click="deleteTarget = { id: $el.dataset.id, name: $el.dataset.name, url: $el.dataset.url }" 
                                            class="p-1.5 text-rose-500 hover:text-rose-700 hover:bg-rose-50 rounded-lg transition text-xs font-bold" 
        ...
        <!-- Single Teleported Dataset-Bound Delete Confirmation Modal -->
        <template x-teleport="body">
            <div x-show="deleteTarget !== null" class="fixed inset-0 z-50 overflow-y-auto bg-navy/60 backdrop-blur-xs flex items-center justify-center p-4" x-cloak>
                <div @click.away="deleteTarget = null" class="card w-full max-w-md bg-white p-6 shadow-2xl rounded-2xl space-y-5 border border-rose-200 relative animate-in fade-in zoom-in duration-150">
                    ...
                    <p class="text-xs text-slate-700 leading-relaxed">
                        Are you sure you want to permanently delete conference <strong class="text-navy font-black" x-text="deleteTarget?.name"></strong>? All associated submissions, checklist items, assigned staff, and settings will be permanently removed.
                    </p>
                    ...
                    <form method="POST" :action="deleteTarget?.url" 
                        ...
                        @csrf
                        @method('DELETE')
                        <button type="button" @click="deleteTarget = null" :disabled="deleting" class="btn btn-secondary text-xs font-bold">Cancel</button>
